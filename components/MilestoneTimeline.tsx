...
export default function MilestoneTimeline({
  ...
  const getStatusColor = (milestone: Milestone) => {
    if (milestone.completedAt || currentValue >= milestone.targetValue) {
      return 'border-green-500 bg-[#f0fafa]'
    }
    if (milestone.status === 'at_risk') {
          ...
          <div key={milestone.id} className="flex items-start gap-4">
            ...
            <div className="flex flex-col items-center">
              ...
              {index < milestones.length - 1 && (
                <div className={`w-0.5 h-12 ${isCompleted ? 'bg-[#f0fafa]0' : 'bg-gray-300'}`} />
              )}
            </div>
            ...
            <div className={`flex-1 p-4 border-2 rounded ${getStatusColor(milestone)}`}>
              <div className="flex items-start justify-between gap-4 mb-2">
                ...
                <div className="text-right">
                  ...
                  {!isCompleted && (
                    <p className={`text-xs font-medium ${isOverdue ? 'text-red-600' : daysUntil < 7 ? 'text-orange-600' : 'text-[#A3A3A3]'}`}>
                      {isOverdue ? `${Math.abs(daysUntil)}d overdue` : `${daysUntil}d remaining`}
                    </p>
                  )}
                  {isCompleted && (
                    <p className="text-xs font-medium text-[#006766]">
                      ✓ Complete
                    </p>
                  )}
                </div>
